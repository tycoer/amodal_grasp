...
def write_voxel_grid(root, scene_id, voxel_grid):
    path = root / "voxel_grid" / (scene_id + ".npz")
...
def write_df(df, root):
    df.to_csv(root / "grasps_new.csv", index=False)
...
def process_one_scene(args, f):
    ...
    write_voxel_grid(args.raw, f.stem, grid)
    ...
    write_point_cloud(args.raw, f.stem, pc)
...
def main(args):
    ...
    (args.raw / "voxel_grid").mkdir(parents=True)
    (args.raw / "point_clouds").mkdir(parents=True)
