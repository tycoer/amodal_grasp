...
class AmodalGripDataset(CustomDataset):
    ...
    def __init__(self,
        ...
        self.scene_process_root = os.path.join(self.data_root, 'scenes_processed')
        ...
        # self.with_scene_process = False
        # if os.path.exists(self.scene_process_root):
        #     self.with_scene_process = True
        #
        #
        # if
        #     # read files
    ...
    def __getitem__(self, item):
        ...
        result = dict(
             # input
             voxel_grid=voxel_grid,       # (40, 40, 40)
             occ_points = occ_points,     # (2048, 3)
             gripper_T = pos.reshape(1, 3),# (1, 3)
             # gt
             gt_width=np.float32(width), # (1,)
             gt_qual=np.float32(label),  # (1,)  binary with 0 and 1
             gt_rotations=np.float32(rotations),     # (2, 4)
             gt_occ=np.float32(occ),       # (2048,) binary with 0 and 1
        ...
        scene_info = dict(np.load(os.path.join(self.scene_process_root, scene_id+'.npz')))
        scene_info['gt_labels'] = np.int64(scene_info['gt_labels'])
        scene_info['gt_bboxes'] = np.float32(scene_info['gt_bboxes'])
        scene_info['ori_shape'] = scene_info['img'].shape
