...
class GraspHead(nn.Module):
    def __init__(self,
        ...
        self.hm_conv = nn.Sequential(*(ConvModule(in_channels, 128, 1),
                                   ConvModule(128, 64, 1),
                                   ConvModule(64, 32, 1),
                                   ConvModule(32, 16, 1),
                                   ConvModule(16, out_channels, 1)))

        self.grasp_conv = nn.Sequential(*(ConvModule(in_channels, 128, 1),
                                   ConvModule(128, 64, 1),
                                   ConvModule(64, 32, 1),
                                   ConvModule(32, 16, 1),
                                   ConvModule(16, out_channels, 1)))
    ...
    def forward_train(self,
        ...
        grasp_feat = self.grasp_conv(x)
        pred_uv_value = batch_index_uv_value(grasp_feat, pred_uv) # shape (bz, 10, 1)
        ...
        loss_qual = self.loss_qual(pred_qual.squeeze(-1), gt_qual.long())
        loss_quat = self.loss_quat(pred_quat, gt_quat)
        loss_width = self.loss_width(pred_width.squeeze(-1), gt_width)
