...
from utils.simulation_test import ClutterRemovalSimWithCategory # tycoer
...
def get_nocs_para(points):
    ...
    norm_factor = norm_factor.tolist()
    corner_pt_left = np.amin(points, axis=0, keepdims=True)
    corner_pt_right = np.amax(points, axis=0, keepdims=True)
    corner_pt = [corner_pt_left, corner_pt_right]
    return norm_factor, corner_pt



# tycoer  修改自 utils/io.write_sensor_data
def write_sensor_data(root, rgb_imgs, depth_imgs, mask_imgs, extrinsics):
...
def get_mesh_pose_list_from_world(world, object_set, exclude_plane=True):
    ...
    for uid in world.bodies.keys():
        ...
        norm_factor, corner_pt = get_nocs_para(np.float32(verts))
        ...
        mesh_info = dict(mesh_path=mesh_path,
                         ...
                         uid=uid,
                         # category=category,
                         nocs_para=dict(norm_factor=norm_factor,
                                        corner_pt=corner_pt),
                         aabb=aabb)
...
def write_point_cloud(root, scene_id, point_cloud, name="point_clouds"):
    ...
    np.savez_compressed(path, point_cloud)
...
def main(args, rank):
    ...
    np.random.seed(seed)
    sim = ClutterRemovalSimWithCategory(args.scene, args.object_set, gui=args.sim_gui)
    ...
    for _ in range(grasps_per_worker // GRASPS_PER_SCENE):
        ...
        # tycoer
        scene_id = write_sensor_data(args.root, rgb_imgs_side, depth_imgs_side, mask_imges_side, extrinsics_side)
