...
def process_single_grasp(dataset_root, obj_idx=66, fric_coef_thresh=0.4):
    ...
    index_all = mask1.flatten().nonzero()[0]
    data = np.hstack((quat, target_points, widths[:, None], depths[:, None], scores[:, None], tolerance[:, None], point_inds[:, None], index_all[:, None]))
    data = np.float32(filter_points_by_max_score(data))
    return data



def process_all_grasp(dataset_root, fric_coef_thresh=0.4):
    save_dir = os.path.join(dataset_root, 'amodal_grasp/grasp_label')
    os.makedirs(save_dir, exist_ok=True)
    max_obj_num = 88
    for obj_idx in tqdm(range((max_obj_num))):
        data = process_single_grasp(dataset_root, obj_idx, fric_coef_thresh)
...
if __name__ == '__main__':
    dataset_root = '/disk5/data/graspnet'
    # process_single_grasp(dataset_root, obj_idx=0)
    process_all_grasp('/disk5/data/graspnet')