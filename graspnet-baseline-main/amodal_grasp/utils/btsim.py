...
from .saver import get_mesh_pose_dict_from_world
...
class Camera(object):
    ...
    def render(self, extrinsic):
        ...
        # result: width, height, rgbaImg, depthImg, segImg
        width, height, rgbaImg, depthImg, segImg = self.p.getCameraImage(
        ...
        # tycoer
        # result[2]: r, g, b, alpha
        if self.p.isNumpyEnabled():
            rgb, z_buffer, mask = rgbaImg[:, :, :3], depthImg, segImg
        else:
            rgba = np.uint8(rgbaImg).reshape(height, width, 4) # shape : (480, 640, 4)
            rgb = rgba[:, :, :3]
            z_buffer = np.float32(depthImg).reshape(height, width)
            mask = np.int32(segImg).reshape(height, width)
        depth = (
