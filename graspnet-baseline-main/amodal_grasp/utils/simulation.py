...
from  .btsim import BtWorld
from .transform import Rotation, Transform
from .misc import apply_noise
...
class ClutterRemovalSim(object):
    def __init__(self, scene, object_set, gui=True, seed=None, add_noise=False, sideview=False, save_dir=None, save_freq=8):
        ...
        self.world = BtWorld(self.gui, save_dir, save_freq)
        self.gripper = Gripper(self.world)
        self.size = 6 * self.gripper.finger_depth
        intrinsic = CameraIntrinsic(480, 480, 540.0, 540.0, 240.0, 240.0)
...
if __name__ == '__main__':
    data_root = 'G:/project/obj_models_new'
    split = 'train'
    scene = 'pile'
    sim_gui = True
    sim = ClutterRemovalSim(object_set='blocks',
                            scene=scene,
                            gui=sim_gui,
                            )
