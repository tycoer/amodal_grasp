...
class VoxelRCNNConvUpsampleHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_channels = 1 if reg_class_agnostic else num_classes
        self.norm  = ''
        ...
        self.predictor = Conv2d(
            conv_dims, self.out_channels * self.num_depth, kernel_size=1, stride=1, padding=0
        )
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.size(0), self.out_channels, self.num_depth, x.size(2), x.size(3))
        return x
    ...
    def get_target(self,
                   ...
                   **kwargs):
        '''
        for i, vox in enumerate(gt_voxels[0]):
            print(vox.shape)
            _ = trimesh.PointCloud(vox).export(f'/home/guest/{i}.ply')
        '''


        voxel_side_len = self.num_depth
        voxel_targets = []
        for sampling_result, voxel, K in zip(sampling_results, gt_voxels, Ks):
            ...
            """
            for i, vox in enumerate(voxel):
                _ = trimesh.PointCloud(vox.cpu()).export(f'/home/guest/gt_{i}.ply')
            """

            '''
            boxes = torch.load('/home/guest/boxes.pth')
            Ks = torch.load('/home/guest/Ks.pth')
            voxels = torch.load('/home/guest/voxels.pth')
            batch_crop_voxels_within_box(voxels=gt_voxels_sampled,
                                         boxes=pos_proposals,
                                         Ks=Ks_sampled,
                                         voxel_side_len=voxel_side_len)
            '''
            if voxel_target.any() == False:
                print(1)
            voxel_targets.append(voxel_target)
        voxel_targets = torch.cat(voxel_targets)
        '''
        import trimesh
        for i, vox in enumerate(voxel_targets.cpu().numpy()):
            if vox.any() == 0:
                print(f'all of target_{i} is 0!')
                continue
            _ = trimesh.PointCloud(np.array(np.where(vox != 0)).T).export(f'/home/guest/{i}.ply')
            if i == 5:
                break
        
        '''
        return voxel_targets
    ...
    def loss(self,
        ...
        loss_voxel = F.binary_cross_entropy_with_logits(voxel_pred, voxel_targets, reduction='mean')
        voxel_accurate = (voxel_pred > 0.5) == (voxel_targets > 0.5)
        voxel_accuracy = voxel_accurate.nonzero().size(0) / voxel_accurate.numel()
        loss_voxel = loss_voxel * self.loss_weight
        return loss_voxel, voxel_accuracy
    ...
    def loss(self,
        ...
        # loss_voxel = sigmoid_focal_loss(voxel_pred, voxel_targets)
        # # loss_voxel = loss_voxel.mean()
        # loss_voxel = (loss_voxel * voxel_targets).mean()
        loss_voxel = F.binary_cross_entropy_with_logits(voxel_pred, voxel_targets, reduction='mean')
        # voxel_accurate = (voxel_pred.sigmoid() > 0.5) == (voxel_targets > 0.5)
        # voxel_accuracy = torch.Tensor([voxel_accurate.nonzero().size(0) / voxel_targets.numel()]).to(loss_voxel.device).detach()
        loss_voxel = loss_voxel * self.loss_weight
        return loss_voxel, 0
