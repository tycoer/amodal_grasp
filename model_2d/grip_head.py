...
from model_2d.utils.encoder import LocalVoxelEncoder
...
class GripHead(nn.Module):
    def __init__(self,
        ...
        self.decoder_qual = LocalDecoder(out_dim=1,**decoder_cfg)
        self.decoder_rot = LocalDecoder(out_dim=4,**decoder_cfg)
        self.decoder_width = LocalDecoder(out_dim=1,**decoder_cfg)
    ...
    def forward(self,
        ...
        if gripper_T is not None and occ_point is None:
            qual, rot, width = self.decode_grip(gripper_T, encoder_features)
            return qual, rot, width
        elif gripper_T is not None and occ_point is not None:
            # detch encoder features
            qual, rot, width = self.decode_grip(gripper_T, encoder_features)
            encoder_features =  {k: v.detach() for k, v in encoder_features.items()}
    ...
    def forward_train(self,
        ...
        if gripper_T is not None and occ_points is None:
            pred_qual, pred_rotation, pred_width = self.forward(voxel_grid_features, gripper_T, occ_points)
            loss_qual, loss_rot, loss_width = self.loss_grip(pred_qual,
                                                             pred_rotation,
                                                             pred_width,
                                                             gt_qual,
                                                             gt_rotation,
            ...
            pred_qual, pred_rotation, pred_width, pred_tsdf = self.forward(voxel_grid_features, gripper_T, occ_points)
